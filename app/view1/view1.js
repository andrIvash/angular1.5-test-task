...
angular.module('myApp.view1', ['ngRoute'])
    ...
    .controller('View1Ctrl', function ($scope) {
        ...
        $scope.myTypeOrder = 'all';

        $scope.typeFilter = function (pokemon) {
            //console.log($scope.myTypeOrder);
           if  ($scope.myTypeOrder !== 'all'){
                var result = false;
                pokemon.type.forEach(function(typeElem) {
                    if(typeElem === $scope.myTypeOrder) {
                        result = true;
                    }
                }); 
                return result;
        ...
        $scope.sayName = function (evt, name) {
            alert(name);
        };
